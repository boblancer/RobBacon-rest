...
def handleBeaconActivity(userID, hwid, timestamp):
    ...
    if User.objects.filter(ID=userID).exists() and Member.objects.filter(classID=hwid).exists() and not Attendance.objects.filter(classID=hwid, userID=userID).exists():
...
class UserList(APIView):
    ...
    def post(self, request, format=None):
        ...
        member_serializer = MemberSerializer(data={"classID": "0136a2e901", "userID": data["ID"]})
